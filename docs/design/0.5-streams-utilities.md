...
Cloned `World` values share the same handler, minimum log level, and feature
registry (same `Arc` pattern as features). Optional `tracing` emission uses
that same minimum level.

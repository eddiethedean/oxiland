...
PyPI wheels. See the [0.7 compatibility report](0.7.md).
...
- [x] Examples run
- [x] Tip CI includes pre-publish gates: version alignment,
  `cargo publish --dry-run`, full 15-wheel validation after install smokes, and
  `cargo audit` (narrow R-020 `quick-xml` ignores only, self-expiring)
- [ ] Tip CI green on main after the R-020 tip exception alignment
...
- [ ] Prefer upgrading Oxigraph when a release accepts `quick-xml >=0.41`
  (clears RUSTSEC-2026-0194/0195; tracked as R-020)

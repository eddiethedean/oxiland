...
Outcome: close the frozen faster-than-Redland gate on every required
target/profile by measuring honestly, fixing the known cliffs, and retaining
0.11 parity on the exact optimized artifacts
...
- Protocol: thresholds and measurement rules in
  [VERIFICATION.md](../VERIFICATION.md#012-performance-optimization) —
  throughput median ratio ≥ `1.05`, latency median ratio ≤ `0.95`, 95%
  bootstrap CI excluding parity, no waiver of required cases.
...
- [ ] Every required sample records production-compile provenance
  (`cargo_profile=release`, `--release --locked`, no debug artifacts)
- [ ] Every required case on every required target/profile passes the
...
- [ ] Independent clean environment reproduces the performance report from
  release builds
